...
> e apagou a versão anterior (Go + regressão _linear_), cujos erros estão
...
| Critério (para o _seu_ objetivo) | Go                                                                                      | Python                                                                            | Vencedor         |
| -------------------------------- | --------------------------------------------------------------------------------------- | --------------------------------------------------------------------------------- | ---------------- |
...
fazer o emissor produzir Go que _compile_ (tipos explícitos, imports usados,
...
- **Algoritmo errado:** tudo é regressão _linear_. Seu objetivo é _logística_.
...
1. **Normalização das features é obrigatória.** Sem padronizar (z-score:
   ...
   exemplo. Use.
2. **Proteja o `log`:** `ln(0)` é `-infinito`. Faça _clipping_ de `ŷ` em
...
> **Detalhe de implementação que você não pode esquecer:** funções que _atribuem_ a
